...
(async () => {
  ...
  try {
    ...
    assert.match(await page.locator('#eventList').innerText(), /ぜにばこキッズラボ/);
    assert.match(await page.locator('#eventList').innerText(), /金唐革紙 箔押し体験会/);
    ...
    for (const width of [320, 375, 390, 768, 1024, 1440]) {
      ...
      if (width === 1440) await page.locator('footer').screenshot({ path: '/private/tmp/otaru-update-footer-desktop.png' });
    }
    await page.setViewportSize({ width: 390, height: 844 });
    await page.locator('footer').screenshot({ path: '/private/tmp/otaru-update-footer-mobile.png' });
    assert((await page.locator('footer').boundingBox()).height < 160, 'collapsed mobile footer should stay compact');
    ...
    assert.equal(await page.locator('header .brand-logo').count(), 0);
    assert.equal(await page.locator('header a').filter({ hasText: 'Spady公式' }).count(), 0);
    ...
    assert.match(await page.locator('#eventList').innerText(), /第77回小樽市文化祭/);
    await page.goto(`${base}/en/events/otaru-yusen-foil-workshop-20261010/`, { waitUntil: 'domcontentloaded' });
    assert.match(await page.locator('h1').innerText(), /Gold-Embossed Decorative Paper Workshop/);
    await page.goto(`${base}/ko/events/otaru-yusen-foil-workshop-20261010/`, { waitUntil: 'domcontentloaded' });
    assert.match(await page.locator('h1').innerText(), /금당혁지/);
    assert.deepEqual(errors, []);
    console.log('Current UI: 6 viewport widths, language picker, compact footer, form and new event details passed.');
  } finally {
    await browser.close();
  }
